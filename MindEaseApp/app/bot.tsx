...
export default function BotScreen() {
  ...
  const fetchHistory = useCallback(async (sessionId: number | null) => {
    if (!token) return;
    if (sessionId === null) {
      setMessages([]);
      return;
  ...
  const deleteSession = async (sessionId: number) => {
    ...
    try {
      await axios.delete(`${API_URL}/bot/sessions/${sessionId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      setSessions((prev: any[]) => prev.filter((s) => Number(s.id) !== Number(sessionId)));
      await fetchSessions();

      Toast.show({
        type: 'success',
        text1: 'Chat deleted',
        text2: 'The selected conversation was removed.',
      });
    } catch (error: any) {
      const msg = error.response?.data?.error || "Could not delete chat.";
      Toast.show({
        type: 'error',
        text1: 'Delete failed',
        text2: msg,
  ...
  const confirmDeleteSession = (sessionId: number, title?: string) => {
    if (Platform.OS === "web" && typeof window !== "undefined") {
      const accepted = window.confirm(`Delete \"${title || 'Untitled Chat'}\"? This cannot be undone.`);
      if (accepted) {
    ...
    Alert.alert(
      'Delete Chat',
      `Delete \"${title || 'Untitled Chat'}\"? This cannot be undone.`,
      [
        ...
        {showSessions ? (
          <View style={styles.sessionsPanelFull}>
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <View style={[styles.sessionItem, currentSessionId === item.id && styles.activeSession]}>
                  <TouchableOpacity style={styles.sessionInfo} onPress={() => selectSession(item.id)}>
                  ...
                  <View style={styles.sessionActions}>
                    <TouchableOpacity
                      onPress={() => confirmDeleteSession(item.id, item.title)}
