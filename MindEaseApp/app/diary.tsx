...
const DiaryHeader = ({
    ...
    <View>
      ...
      {!isWritingMode && filteredEntries.length > 0 && (
  ...
  const fetchEntries = async (query = "") => {
    ...
    try {
      ...
      const response = await axios.get(
        trimmedQuery ? `${API_URL}/diary/search` : `${API_URL}/diary`,
        {
  ...
  return (
    ...
    >
      <SafeAreaView style={styles.container} edges={['top', 'left', 'right']}>
        ...
        >
          {showDatePicker ? (
            <View style={styles.pickerContainerOverlay}>
                <View style={styles.datePickerExpanded}>
                    ...
                    <TouchableOpacity
                        style={[styles.confirmDateBtn, { backgroundColor: '#94a3b8', marginTop: 10, borderBottomColor: '#475569' }]}
                        onPress={() => setShowDatePicker(false)}
                    >
                        <Text style={styles.confirmDateText}>CANCEL</Text>
                    </TouchableOpacity>
                </View>
            </View>
          ) : (
            <FlatList
                ref={flatListRef}
                data={isWritingMode ? [] : filteredEntries}
